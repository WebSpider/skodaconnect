...
class APIClient(ABC):
    ...
    async def skoda_token(
        ...
        try:
            ...
            if not all(
                token in tokens for token in [IDTOKEN, ACCESSTOKEN, REFRESHTOKEN]
            ):
    ...
    async def model_image(
        self: APIClient, vin: str, size: str = LARGE
    ) -> Union(str, None):
        """
        ...
        try:
            ...
            digest = hmac.new(
                MODELAPIKEY, msg=to_sign.encode(), digestmod=hashlib.sha256
            ).digest()
