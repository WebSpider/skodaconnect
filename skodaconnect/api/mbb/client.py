...
class MBBClient(ConnectClient):
    ...
    def __init__(
        self: ConnectClient, http_session: ClientSession, api_debug: bool = False
    ) -> None:
        """
    ...
    async def revoke_mbb_token(self: ConnectClient, typ: str = ACCESS_TOKEN) -> bool:
        ...
        try:
            ...
            revoke_resp = await self._request(
                url=revoke_url, data={TOKEN: token, TOKENHINT: typ}
            )
    ...
    async def garage(self: ConnectClient) -> dict:
        ...
        try:
            response = await self._mbb_api_call(
                GARAGE_URL.format(BRAND=BRAND, COUNTRY=COUNTRY)
            )
    ...
    async def operation_list(
        self: ConnectClient, vin: str, base: str = BASE_URL
    ) -> dict:
    ...
    async def _mbb_set(
        self: ConnectClient, url: str, payload: dict, headers: dict = None
    ) -> dict:
        """
    ...
    async def charge_current(
        ...
        try:
            ...
            req_payload = {
                DATA: {ACTION: {SETTINGS: {MAXCURRENT: int(value)}, TYPE: SETSETTINGS}}
            }
            return await self._set_charger(vin=vin, payload=req_payload, base=base)
        except Exception as exc:  # pylint: disable=broad-except
            return {ERROR: exc}
