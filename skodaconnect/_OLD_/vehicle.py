...
class Vehicle:
    def __init__(self, conn, data):
        ...
        # SmartLink connectivity is enabled
            ...
            self._services.update({"vehicle_status": {"active": True}})
    ...
    async def discover(self):
        ...
        if "ONLINE" in self._connectivities:
            ...
            await asyncio.gather(self.get_realcardata(), return_exceptions=True)
            _LOGGER.debug(
                "Attempting discovery of supported API endpoints for vehicle."
            )
                ...
                for service in serviceInfo:
                    try:
                        if service.get("serviceId", "Invalid") in self._services.keys():
                            ...
                            if (
                                service.get("serviceStatus", {}).get(
                                    "status", "Disabled"
                                )
                                == "Enabled"
                            ):
                                data["active"] = True
                                if service.get("cumulatedLicense", {}).get(
                                    "expirationDate", False
                                ):
                                    data["expiration"] = (
                                        service.get("cumulatedLicense", {})
                                        .get("expirationDate", None)
                                        .get("content", None)
                                    )
                                ...
                                _LOGGER.debug(
                                    f'Discovered active supported service: {serviceName}, licensed until {data.get("expiration").strftime("%Y-%m-%d %H:%M:%S")}'
                                )
                            elif (
                                service.get("serviceStatus", {}).get("status", None)
                                == "Disabled"
                            ):
                                reason = service.get("serviceStatus", {}).get(
                                    "reason", "Unknown"
                                )
                                _LOGGER.debug(
                                    f"Service: {serviceName} is disabled because of reason: {reason}"
                                )
                                data["active"] = False
                            else:
                                _LOGGER.warning(
                                    f"Could not determine status of service: {serviceName}, assuming enabled"
                                )
                                data["active"] = True
                            self._services[serviceName].update(data)
                    except Exception as error:
                        _LOGGER.warning(
                            f'Encountered exception: "{error}" while parsing service item: {service}'
                        )
                        pass
            else:
                _LOGGER.warning(
                    f"Could not determine available API endpoints for {self.vin}"
                )
            if self._connection._session_fulldebug:
                for endpointName, endpoint in self._services.items():
                    if endpoint.get("active", False):
                        _LOGGER.debug(
                            f'API endpoint "{endpointName}" valid until {endpoint.get("expiration").strftime("%Y-%m-%d %H:%M:%S")} - operations: {endpoint.get("operations", [])}'
                        )
                ...
                if data:
                    ...
                    if data.get("findCarResponse", {}).get("parkingTimeUTC", False):
                        try:
                            ...
                            oldTime = self.attrs.get("findCarResponse").get(
                                "parkingTimeUTC"
                            )
                            if newTime > oldTime:
            ...
            if data:
                ...
                if "errors" in data:
                    for e in data.get("errors", []):
                        _LOGGER.info(
                            f"Could not fetch {e.get('type', '')}, error description: {e.get('description', '')}"
                        )
                        # Use stored data for sections with errors
                        if e.get("type", "") == "MILEAGE_LOAD_FAILED":
                            data["vehicle_remote"]["mileageInKm"] = self._states.get(
                                "vehicle_remote", {}
                            ).get("mileageInKm", {})
                        if e.get("type", "") == "DOORS_LOAD_FAILED":
                            data["vehicle_remote"]["doors"] = self._states.get(
                                "vehicle_remote", {}
                            ).get("doors", {})
                            data["vehicle_remote"]["status"] = self._states.get(
                                "vehicle_remote", {}
                            ).get("status", {})
                        if e.get("type", "") == "WINDOWS_LOAD_FAILED":
                            data["vehicle_remote"]["windows"] = self._states.get(
                                "vehicle_remote", {}
                            ).get("windows", {})
                        if e.get("type", "") == "PARKING_LIGHTS_LOAD_FAILED":
                            data["vehicle_remote"]["lights"] = self._states.get(
                                "vehicle_remote", {}
                            ).get("lights", {})
                self._states.update(data)
            else:
    ...
    async def wait_for_request(self, section, request, retryCount=36):
        ...
        if retryCount == 0:
            ...
            return "Timeout"
        try:
            status = await self._connection.get_request_status(
                self.vin, section, request
            )
            ...
            _LOGGER.warning(
                f"Exception encountered while waiting for request status: {error}"
            )
            return "Exception"
    ...
    async def set_charger_current(self, value):
        ...
        if self.is_charging_supported:
            ...
            if isinstance(value, int):
                if 1 <= int(value) <= 255:
                    ...
                    # Skoda Native API charger current request, does this work?
                        ...
                        data = {
                            "chargingSettings": {
                                "autoUnlockPlugWhenCharged": self.attrs.get(
                                    "chargerSettings", {}
                                ).get("autoUnlockPlugWhenCharged", "Off"),
                                "maxChargeCurrentAc": value,
                                "targetStateOfChargeInPercent": self.attrs.get(
                                    "chargerSettings", {}
                                ).get("targetStateOfChargeInPercent", 100),
                            },
                            "type": "UpdateSettings",
                        }
                else:
            ...
            # Mimick app and set charger max ampere to Maximum/Reduced
                ...
                _LOGGER.error("Data type passed is invalid.")
                raise SkodaInvalidRequestException("Invalid data type.")
    ...
    async def set_charger(self, action):
        ...
        if not self._services.get(
            "rbatterycharge_v1", False
        ) and not self._services.get("CHARGING", False):
        ...
        if self._requests["batterycharge"].get("id", False):
            timestamp = self._requests.get("batterycharge", {}).get(
            ...
            if expired > timestamp:
                ...
                raise SkodaRequestInProgressException(
        ...
        if self._services.get("CHARGING", False):
            if action in ["start", "stop"]:
                ...
                _LOGGER.error(
                    f"Invalid charger action: {action}. Must be one of start, stop or data for updating settings"
                )
                raise SkodaInvalidRequestException(
                    f"Invalid charger action: {action}. Must be one of start, stop or data for set chargelimit"
                )
        try:
            ...
            if not response:
                ...
                if response.get("state", None) == "Throttled":
                    ...
                    if self._services.get("rbatterycharge_v1", False):
                        status = await self.wait_for_request(
                            "batterycharge", response.get("id", 0)
                        )
                    elif self._services.get("CHARGING", False):
                        status = await self.wait_for_request(
                            "charging", response.get("id", 0)
                        )
        ...
        # Skoda Native API charging
            ...
            if isinstance(limit, int):
                if limit in [50, 60, 70, 80, 90, 100]:
                    ...
                    raise SkodaInvalidRequestException(
                        "Charge limit must be one of 50, 60, 70, 80, 90 or 100."
                    )
            else:
                raise SkodaInvalidRequestException(
                    f'Charge limit "{limit}" is not supported.'
                ...
                try:
                    ...
                    for timer in data["timersSettings"]["timers"]:
                        ...
                        if timer.get("id", None) == id:
                            ...
                            data["timersSettings"]["timers"][index]["enabled"] = (
    ...
    async def set_heatersource(self, source="electric", spin=None):
        ...
        if not self.is_auxiliary_climatisation_supported:
            raise SkodaInvalidRequestException(
                "Auxiliary heater is not supprted on this vehicle."
            )
        if spin is None:
        ...
        if self._services.get("timerprogramming_v1", False):
            if source.lower() in ["electric", "automatic"]:
                ...
                raise SkodaInvalidRequestException(
                    f'Source {data.get("action", "")} is not supported as heater source.'
                )
            return await self._set_timers(data)
        else:
            raise SkodaInvalidRequestException("Departure timers are not supported.")
    ...
    async def set_timer_schedule(self, id, schedule={}, spin=False):
        ...
        if not schedule.get("heaterSource", False):
            if schedule.get("heaterSource", None) not in ["automatic", "electric"]:
                raise SkodaInvalidRequestException(
                    'Heater source must be one of "electric" or "automatic".'
                )
        elif spin is False:
            if schedule.get("heaterSource", None) == "automatic":
                raise SkodaInvalidRequestException(
                    'SPIN must be supplied when using auxiliary heater".'
                )
            ...
            if schedule.get("chargeMaxCurrent", None) is not None:
                if isinstance(schedule.get("chargeMaxCurrent", None), str):
                    if schedule.get("chargeMaxCurrent", None) not in [
                        ...
                        if schedule.get("chargeMaxCurrent", None) in [
                            "Maximum",
                            "maximum",
                            "Max",
                            "max",
                        ]:
                            schedule["chargeMaxCurrent"] = 254
        ...
        # Skoda native API
            ...
            try:
                ...
                for timer in data["timersSettings"]["timers"]:
                    if timer.get("id", None) == id:
                        index = data["timersSettings"]["timers"].index(timer)
                        data["timersSettings"]["timers"][index][
                            "enabled"
                        ] = schedule.get("enabled")
                        data["timersSettings"]["timers"][index]["time"] = schedule.get(
                            "time"
                        )
                ...
                _LOGGER.debug(f"Exception: {e}")
                pass
            raise SkodaInvalidRequestException("Timer action failed. ")
        else:
    ...
    async def _set_timers(self, data=None):
        ...
        if self._requests["departuretimer"].get("id", False):
            timestamp = self._requests.get("departuretimer", {}).get(
                "timestamp", datetime.now()
            )
            expired = datetime.now() - timedelta(minutes=3)
            if expired > timestamp:
        ...
        try:
            ...
            response = await self._connection.setDeparturetimer(
                self.vin, data, spin=data.get("spin", False)
            )
            if not response:
                ...
                if response.get("state", None) == "Throttled":
                    ...
                    status = await self.wait_for_request(
                        "departuretimer", response.get("id", 0)
                    )
    ...
    async def set_climatisation_temp(self, temperature=20):
        ...
        if (
            ...
            if 16 <= float(temperature) <= 30:
                ...
                _LOGGER.error(
                    f"Set climatisation target temp to {temperature} is not supported."
                )
                raise SkodaInvalidRequestException(
                    f"Set climatisation target temp to {temperature} is not supported."
                )
        ...
        if hasattr(self, supported):
            if action in ["enable", "disable"]:
                ...
                if self.attrs.get("airConditioningSettings", False):
                    data = {
                        "airConditioningSettings": self.attrs.get(
                    ...
                    _LOGGER.warning(
                        "Could not find stored climatisation settings, using defaults."
                    )
                    data = {
                        "airConditioningSettings": {
                ...
                data["airConditioningSettings"]["zonesSettings"][
                    f"{seat}Enabled"
                ] = setting
                return await self._set_aircon(data)
            else:
                ...
                raise SkodaInvalidRequestException(
                    f'Seat heating action "{action}" is not supported.'
                )
        else:
            _LOGGER.error("No climatisation support.")
            raise SkodaInvalidRequestException("No climatisation support.")
    ...
    async def set_aircon_at_unlock(self, setting=False):
        ...
        if self.is_electric_climatisation_supported:
            if isinstance(setting, bool):
                ...
                _LOGGER.error(
                    f'Setting air-conditioning at unlock to "{setting}" is not supported.'
                )
                raise SkodaInvalidRequestException(
                    f'Setting air-conditioning at unlock to "{setting}" is not supported.'
                )
        ...
        if self.is_window_heater_supported or self.is_window_heater_new_supported:
            if action in ["start", "stop", "enabled", "disabled"]:
                ...
                if self._services.get("AIR_CONDITIONING", False):
                    if action in ["start", "stop"]:
                        data = {"type": action, "section": "WindowHeating"}
                        return await self._set_aircon(data)
    ...
    async def set_battery_climatisation(self, mode=False):
        ...
        if self.is_electric_climatisation_supported:
            if mode in [True, False]:
                data = {
                    "action": {
                        "settings": {"climatisationWithoutHVpower": mode},
                        "type": "setSettings",
                    }
                }
            else:
                _LOGGER.error(
                    f'Set climatisation without external power to "{mode}" is not supported.'
                )
                raise SkodaInvalidRequestException(
                    f'Set climatisation without external power to "{mode}" is not supported.'
                )
    ...
    async def set_climatisation(self, mode="off", temp=None, hvpower=None, spin=None):
        ...
        if temp is not None:
            if not isinstance(temp, float):
                raise SkodaInvalidRequestException("Invalid type for temp")
            elif not 16 <= float(temp) <= 30:
                raise SkodaInvalidRequestException("Invalid value for temp")
        else:
            temp = self.climatisation_target_temperature
        if hvpower is not None:
            if not isinstance(hvpower, bool):
                raise SkodaInvalidRequestException("Invalid type for hvpower")
        if self.is_electric_climatisation_supported:
            if self._services.get("rclima_v1", False):
                ...
                if mode == "auxiliary":
                    raise SkodaInvalidRequestException(
                        "No auxiliary climatisation support."
                    )
                if mode in ["Start", "start", "On", "on", "electric"]:
                    ...
                    if airconData:
                        ...
                        if self.attrs.get("airConditioningSettings", False):
                            _LOGGER.warning(
                                "Failed to fetch climatisation settings, using saved values."
                            )
                            data = {
                                "airConditioningSettings": self.attrs.get(
                                    "airConditioningSettings"
                            ...
                            _LOGGER.warning(
                                "Could not fetch climatisation settings, using defaults."
                            )
                            data = {
                                "airConditioningSettings": {
                                    "targetTemperatureInKelvin": 294.15,
    ...
    async def _set_climater(self, data, spin=False):
        ...
        if self._requests["climatisation"].get("id", False):
            timestamp = self._requests.get("climatisation", {}).get(
                "timestamp", datetime.now()
            )
            expired = datetime.now() - timedelta(minutes=3)
            if expired > timestamp:
        ...
        try:
            ...
            if not response:
                ...
                if response.get("state", None) == "Throttled":
                    ...
                    status = await self.wait_for_request(
                        "climatisation", response.get("id", 0)
                    )
    ...
    async def _set_aircon(self, data, spin=False):
        ...
        if not self._services.get("AIR_CONDITIONING", False):
            _LOGGER.info(
                "Remote control of air conditioning functions is not supported."
            )
            raise SkodaInvalidRequestException(
                "Remote control of air conditioning functions is not supported."
            )
        if self._requests["air-conditioning"].get("id", False):
            timestamp = self._requests.get("air-conditioning", {}).get(
                "timestamp", datetime.now()
            )
            expired = datetime.now() - timedelta(minutes=3)
            if expired > timestamp:
                ...
                raise SkodaRequestInProgressException(
                    "Air conditioning action is already in progress"
                )
        try:
            ...
            if not response:
                ...
                if response.get("state", None) == "Throttled":
                    ...
                    status = await self.wait_for_request(
                        "air-conditioning", response.get("id", 0)
                    )
    ...
    async def set_pheater(self, mode, spin):
        ...
        if self._requests["preheater"].get("id", False):
            timestamp = self._requests.get("preheater", {}).get(
                "timestamp", datetime.now()
            )
            expired = datetime.now() - timedelta(minutes=3)
            if expired > timestamp:
    ...
    async def set_lock(self, action, spin):
        ...
        if self._requests["lock"].get("id", False):
            timestamp = self._requests.get("lock", {}).get(
    ...
    async def set_honkandflash(self, action, lat=None, lng=None):
        ...
        if self._requests["honkandflash"].get("id", False):
            timestamp = self._requests.get("honkandflash", {}).get(
                "timestamp", datetime.now() - timedelta(minutes=5)
            )
            expired = datetime.now() - timedelta(minutes=3)
            if expired > timestamp:
                ...
                raise SkodaRequestInProgressException(
                    "A honk and flash action is already in progress"
                )
        if action == "flash":
        ...
        try:
            ...
            if lat is None:
                lat = int(
                    self.attrs.get("findCarResponse", {})
                    .get("Position", {})
                    .get("carCoordinate", {})
                    .get("latitude", None)
                )
            ...
            if not response:
                ...
                _LOGGER.error("Failed to execute honk and flash action")
                raise SkodaException("Failed to execute honk and flash action")
            else:
                self._requests["remaining"] = response.get("rate_limit_remaining", -1)
    ...
    async def set_refresh(self):
        ...
        if self._requests["refresh"].get("id", False):
            timestamp = self._requests.get("refresh", {}).get(
                "timestamp", datetime.now() - timedelta(minutes=5)
            )
            expired = datetime.now() - timedelta(minutes=3)
            if expired > timestamp:
                ...
                raise SkodaRequestInProgressException(
                    "A data refresh request is already in progress"
                )
        try:
    ...
    async def expired(self, service):
        ...
        try:
            ...
            if self._services.get(service, {}).get("expiration", False):
                ...
                _LOGGER.debug(
                    f"Could not determine end of access for service {service}, assuming it is valid"
                )
                expiration = datetime.utcnow() + timedelta(days=1)
            ...
            _LOGGER.debug(
                f"Exception. Could not determine end of access for service {service}, assuming it is valid"
            )
            return False
    ...
    def model(self):
        ...
        if self._specification.get("trimLevel", False):
            model = (
                self._specification.get("title", "Unknown")
                + " "
                + self._specification.get("trimLevel", "")
            )
            return model
    ...
    def parking_light(self):
        ...
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301010001"].get(
                    "value", 0
                )
            )
    ...
    def is_parking_light_supported(self):
        ...
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            if "0x0301010001" in self.attrs.get("StoredVehicleDataResponseParsed"):
                ...
                return False
        if self.attrs.get("vehicle_remote", {}):
            if "overallStatus" in self.attrs.get("vehicle_remote", {}).get(
                "lights", {}
            ):
                return True
            else:
    ...
    def last_connected(self):
        ...
        if self.attrs.get("StoredVehicleDataResponse", False):
            ...
            last_connected_utc = self.attrs.get("vehicle_remote", {}).get(
                "capturedAt", None
            )
            if isinstance(last_connected_utc, datetime):
                last_connected = last_connected_utc.astimezone().replace(tzinfo=None)
    ...
    def distance(self):
        ...
        if self.attrs.get("vehicle_status", False):
            ...
            value = self.attrs.get("StoredVehicleDataResponseParsed")[
                "0x0101010002"
            ].get("value", 0)
        if value:
            return int(value)
    ...
    def service_inspection_distance(self):
        ...
        if self.attrs.get("vehicle_status", {}).get("nextInspectionDistance", False):
            value = self.attrs.get("vehicle_status", {}).get(
                "nextInspectionDistance", 0
            )
        elif (
            self.attrs.get("StoredVehicleDataResponseParsed", {})
            .get("0x0203010003", {})
    ...
    def is_adblue_level_supported(self):
        ...
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            if "0x02040C0001" in self.attrs.get("StoredVehicleDataResponseParsed"):
                if (
                    "value"
                    in self.attrs.get("StoredVehicleDataResponseParsed")["0x02040C0001"]
                ):
                    if (
                        self.attrs.get("StoredVehicleDataResponseParsed")[
                            "0x02040C0001"
        ...
        if self.attrs.get("charger", False):
            if "status" in self.attrs.get("charger", {}):
                if "chargingStatusData" in self.attrs.get("charger")["status"]:
                    if (
                        "chargingState"
                        in self.attrs.get("charger")["status"]["chargingStatusData"]
                    ):
                        return True
        elif self.attrs.get("charging", False):
            return True
    ...
    def min_charge_level(self):
        ...
        if (
            self.attrs.get("departuretimer", {})
            .get("timersAndProfiles", {})
            .get("timerBasicSetting", {})
            .get("chargeMinLimit", False)
        ):
            return (
                self.attrs.get("departuretimer", {})
                .get("timersAndProfiles", {})
                .get("timerBasicSetting", {})
                .get("chargeMinLimit", 0)
            )
        elif self.attrs.get("chargerSettings", False):
            return self.attrs.get("chargerSettings", {}).get(
                "targetStateOfChargeInPercent", 0
            )
        else:
            return 0
    ...
    def is_min_charge_level_supported(self):
        ...
        if (
            ...
            return True
        elif self.attrs.get("chargerSettings", {}).get(
            "targetStateOfChargeInPercent", False
        ):
            return True
        return False
    ...
    def plug_autounlock(self):
        ...
        if (
            self.attrs.get("chargerSettings", {}).get("autoUnlockPlugWhenCharged", None)
            == "Permanent"
        ):
            return True
        return False
    ...
    def is_plug_autounlock_supported(self):
        ...
        if self.attrs.get("chargerSettings", {}).get(
        ...
        if self.attrs.get("charger", False):
            if "status" in self.attrs.get("charger"):
                if "batteryStatusData" in self.attrs.get("charger")["status"]:
                    if (
                        "stateOfCharge"
                        in self.attrs.get("charger")["status"]["batteryStatusData"]
                    ):
                        return True
        elif self.attrs.get("battery", False):
    ...
    def charge_max_ampere(self):
        ...
        if self.attrs.get("charger", False):
            ...
            value = self.attrs.get("chargerSettings", {}).get(
                "maxChargeCurrentAc", "Unknown"
            )
            return value
        return 0
    ...
    def charging_cable_locked(self):
        ...
        response = ""
        if self.attrs.get("charger", False):
            response = self.attrs.get("charger")["status"]["plugStatusData"][
                "lockState"
            ].get("content", 0)
        ...
        if self.attrs.get("charger", False):
            if "status" in self.attrs.get("charger", {}):
                if "plugStatusData" in self.attrs.get("charger").get("status", {}):
                    if "lockState" in self.attrs.get("charger")["status"].get(
                        "plugStatusData", {}
                    ):
                        return True
        elif self.attrs.get("plug", False):
    ...
    def is_charging_cable_connected_supported(self):
        ...
        if self.attrs.get("charger", False):
            if "status" in self.attrs.get("charger", {}):
                if "plugStatusData" in self.attrs.get("charger").get("status", {}):
                    if "plugState" in self.attrs.get("charger")["status"].get(
                        "plugStatusData", {}
                    ):
                        return True
    ...
    def charging_time_left(self):
        ...
        if self.external_power:
            if self.attrs.get("charging", {}).get(
                "remainingToCompleteInSeconds", False
            ):
                minutes = (
                    int(
                        self.attrs.get("charging", {}).get(
                ...
                .get("status", {})
                .get("batteryStatusData", {})
                .get("remainingChargingTime", False)
            ):
                minutes = (
                    self.attrs.get("charger", {})
                    .get("status", {})
    ...
    def is_charging_power_supported(self):
        ...
        if self.attrs.get("charging", False):
            if (
                self.attrs.get("charging", {}).get("chargingPowerInWatts", False)
                is not False
            ):
                return True
        return False
    ...
    def charge_rate(self):
        ...
        if self.attrs.get("charging", False):
            return int(
                self.attrs.get("charging", {}).get("chargingRateInKilometersPerHour", 0)
            )
        else:
            return 0
    ...
    def is_charge_rate_supported(self):
        ...
        if self.attrs.get("charging", False):
            if (
                self.attrs.get("charging", {}).get(
                    "chargingRateInKilometersPerHour", False
                )
                is not False
            ):
                return True
    ...
    def external_power(self):
        ...
        if self.attrs.get("charger", False):
            ...
            response = (
                "Charging"
                if self.attrs.get("charging", {}).get("chargingType", "Invalid")
                != "Invalid"
                else "Invalid"
            )
        return (
            True if response in ["stationConnected", "available", "Charging"] else False
        )
    ...
    def position(self):
        ...
        try:
            if self.vehicle_moving:
                ...
                if self._services.get("carfinder_v1", {}).get("active", False):
                    lat = (
                        int(posObj.get("Position").get("carCoordinate").get("latitude"))
                        / 1000000
                    )
                    lng = (
                        int(
                            posObj.get("Position").get("carCoordinate").get("longitude")
                        )
                        / 1000000
                    )
                ...
                output = {"lat": lat, "lng": lng, "timestamp": parkingTime}
    ...
    def primary_range(self):
        ...
        if "0x0301030006" in self.attrs.get("StoredVehicleDataResponseParsed"):
            if (
                "value"
                in self.attrs.get("StoredVehicleDataResponseParsed")["0x0301030006"]
            ):
                value = self.attrs.get("StoredVehicleDataResponseParsed")[
                    "0x0301030006"
                ].get("value", 0)
    ...
    def primary_drive(self):
        ...
        if "0x0301030007" in self.attrs.get("StoredVehicleDataResponseParsed"):
            if (
                "value"
                in self.attrs.get("StoredVehicleDataResponseParsed")["0x0301030007"]
            ):
                value = self.attrs.get("StoredVehicleDataResponseParsed")[
                    "0x0301030007"
                ].get("value", 0)
    ...
    def secondary_range(self):
        ...
        if "0x0301030008" in self.attrs.get("StoredVehicleDataResponseParsed"):
            if (
                "value"
                in self.attrs.get("StoredVehicleDataResponseParsed")["0x0301030008"]
            ):
                value = self.attrs.get("StoredVehicleDataResponseParsed")[
                    "0x0301030008"
                ].get("value", 0)
    ...
    def secondary_drive(self):
        ...
        if "0x0301030009" in self.attrs.get("StoredVehicleDataResponseParsed"):
            if (
                "value"
                in self.attrs.get("StoredVehicleDataResponseParsed")["0x0301030009"]
            ):
                value = self.attrs.get("StoredVehicleDataResponseParsed")[
                    "0x0301030009"
                ].get("value", 0)
    ...
    def electric_range(self):
        ...
        if self.is_secondary_drive_supported:
            ...
            value = (
                int(
                    self.attrs.get("battery", {}).get(
                        "cruisingRangeElectricInMeters", 0
                    )
                )
                / 1000
            )
    ...
    def fuel_level(self):
        ...
        if self.attrs.get("vehicle_status", False):
            value = round(
                100 * self.attrs.get("vehicle_status", {}).get("primaryFuelLevel", 0)
            )
        elif "0x030103000A" in self.attrs.get("StoredVehicleDataResponseParsed"):
            if (
                "value"
                in self.attrs.get("StoredVehicleDataResponseParsed")["0x030103000A"]
            ):
                value = self.attrs.get("StoredVehicleDataResponseParsed")[
                    "0x030103000A"
                ].get("value", 0)
    ...
    def climatisation_target_temperature(self):
        ...
        if self.attrs.get("climater", False):
            ...
            value = (
                float(
                    self.attrs.get("airConditioningSettings").get(
                        "targetTemperatureInKelvin", 273.15
                    )
                    - 0.15
                )
                * 10
    ...
    def is_climatisation_target_temperature_supported(self):
        ...
        if self.attrs.get("climater", False):
            ...
            if "targetTemperatureInKelvin" in self.attrs.get(
                "airConditioningSettings", {}
            ):
                return True
        return False
    ...
    def climatisation_time_left(self):
        ...
        if self.attrs.get("airConditioning", {}).get(
            "remainingTimeToReachTargetTemperatureInSeconds", False
        ):
            try:
                minutes = (
                    int(
                        self.attrs.get("airConditioning", {}).get(
        ...
        if self.attrs.get("climater", False):
            if "settings" in self.attrs.get("climater", {}):
                if (
                    ...
                    in self.attrs.get("climater", {})["settings"]
                ):
                    return True
            else:
                return False
    ...
    def electric_climatisation(self):
        ...
        if (
            ...
            if (
                status in ["heating", "cooling", "ventilation", "on"]
                and climatisation_type == "electric"
            ):
                return True
        elif self.attrs.get("airConditioning", {}).get("state", "off").lower() in [
            "on",
            "heating",
            "cooling",
            "ventilation",
    ...
    def auxiliary_climatisation(self):
        ...
        if (
            status in ["heating", "cooling", "ventilation", "heatingAuxiliary", "on"]
            and climatisation_type == "auxiliary"
        ):
            return True
    ...
    def is_aux_heater_for_departure_supported(self):
        ...
        if (
            self.is_departure1_supported
            and self.is_electric_climatisation_supported
            and self.is_auxiliary_climatisation_supported
        ):
            return True
        return False
    ...
    def aircon_at_unlock(self):
        """Return status of air-conditioning at unlock setting."""
        return self.attrs.get("airConditioningSettings", {}).get(
            "airConditioningAtUnlock", False
        )
    ...
    def is_aircon_at_unlock_supported(self):
        ...
        if self.attrs.get("airConditioningSettings", {}).get(
            "airConditioningAtUnlock", False
        ):
            return True
        return False
    ...
    def window_heater_new(self):
        ...
        if self.attrs.get("airConditioning", {}).get("windowsHeatingStatuses", False):
            status = self.attrs.get("airConditioning", {}).get(
    ...
    def is_window_heater_supported(self):
        ...
        if self.is_electric_climatisation_supported:
            if self.attrs.get("climater", False):
                if self.attrs.get("climater", {}).get("status", {}).get(
                    "windowHeatingStatusData", {}
                ).get("windowHeatingStateFront", {}).get("content", "") in [
                    "on",
                    "off",
                ]:
                    return True
                if self.attrs.get("climater", {}).get("status", {}).get(
                    "windowHeatingStatusData", {}
                ).get("windowHeatingStateRear", {}).get("content", "") in ["on", "off"]:
                    return True
        return False
    ...
    def window_heater_attributes(self):
        ...
        if self.attrs.get("climater", False):
            ...
            if self.attrs.get("airConditioning", {}).get(
                ...
                # return self.attrs.get('airConditioningSettings', {}).get('windowsHeatingEnabled', False)
                statuses = self.attrs.get("airConditioning", {}).get(
                    "windowsHeatingStatuses", {}
                )
    ...
    def seat_heating_front_left(self):
        ...
        return (
            self.attrs.get("airConditioningSettings", {})
            .get("zonesSettings", {})
            .get("frontLeftEnabled", False)
        )
    ...
    def window_closed_left_front(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301050001"].get(
                    "value", 0
                )
            )
    ...
    def is_window_closed_left_front_supported(self):
        ...
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            ...
            window = next(item for item in windows if item["name"] == "FRONT_LEFT")
            return (
                True if window.get("status", "UNSUPPORTED") != "UNSUPPORTED" else False
            )
    ...
    def window_closed_right_front(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301050005"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def window_closed_left_back(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301050003"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def window_closed_right_back(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301050007"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def sunroof_closed(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x030105000B"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def is_sunroof_closed_supported(self):
        ...
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            ...
            return (
                True if sunroof.get("status", "UNSUPPORTED") != "UNSUPPORTED" else False
            )
    ...
    def door_locked(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            # LEFT FRONT
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301040001"].get(
                    "value", 0
                )
            )
            if response != 2:
            ...
            # LEFT REAR
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301040004"].get(
                    "value", 0
                )
            )
            if response != 2:
            ...
            # RIGHT FRONT
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301040007"].get(
                    "value", 0
                )
            )
            if response != 2:
            ...
            # RIGHT REAR
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x030104000A"].get(
                    "value", 0
                )
            )
            if response != 2:
            ...
            response = (
                self.attrs.get("vehicle_remote", {}).get("status", {}).get("locked", 0)
            )
    ...
    def trunk_locked(self):
        response = int(
            self.attrs.get("StoredVehicleDataResponseParsed")["0x030104000D"].get(
                "value", 0
            )
        )
        if response == 2:
    ...
    def hood_closed(self):
        ...
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301040011"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
            ...
            if doors is not None:
                ...
                return (
                    True
                    if bonnet.get("status", "UNSUPPORTED") in ["CLOSED", "LOCKED"]
                    else False
                )
        return False
    ...
    def is_hood_closed_supported(self):
        ...
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            ...
            bonnet = next(item for item in doors if item["name"] == "BONNET")
            return (
                True if bonnet.get("status", "UNSUPPORTED") != "UNSUPPORTED" else False
            )
    ...
    def door_closed_left_front(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301040002"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
            ...
            return (
                True
                if door.get("status", "UNSUPPORTED") in ["CLOSED", "LOCKED"]
                else False
            )
    ...
    def door_closed_right_front(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301040008"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def door_closed_left_back(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x0301040005"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def door_closed_right_back(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x030104000B"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def trunk_closed(self):
        if self.attrs.get("StoredVehicleDataResponseParsed", False):
            response = int(
                self.attrs.get("StoredVehicleDataResponseParsed")["0x030104000E"].get(
                    "value", 0
                )
            )
            return True if response == 3 else False
    ...
    def is_trip_last_average_electric_consumption_supported(self):
        ...
        if response and type(
            response.get("averageElectricEngineConsumption", None)
        ) in (float, int):
            return True
    ...
    def is_trip_last_average_fuel_consumption_supported(self):
        response = self.trip_last_entry
        if response and type(response.get("averageFuelConsumption", None)) in (
            float,
            int,
        ):
            return True
    ...
    def is_trip_last_average_auxillary_consumption_supported(self):
        response = self.trip_last_entry
        if response and type(response.get("averageAuxiliaryConsumption", None)) in (
            float,
            int,
        ):
            return True
        ...
        for section in self._requests:
            if section in [
                ...
                if self._requests.get(section, {}).get("timestamp", None) is None:
                    _LOGGER.debug(
                        f"Failed to get latest timestamp for {section} request."
                    )
